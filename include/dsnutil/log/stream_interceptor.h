...
public:
    ...
    stream_interceptor(std::ostream& stream,
        ...
        m_orgstream(stream), m_name("unnamed std::ostream"),
        m_logger(logger), m_severity(severity)
    {
        init(name);
    }
    ...
    stream_interceptor(std::ostream& stream, const std::string& name, boost::log::trivial::severity_level severity=boost::log::trivial::trace) :
        m_orgstream(stream),
        m_loggerPtr(new boost::log::sources::severity_logger<levelT>), m_logger(*m_loggerPtr.get()),
        m_name("unnamed std::ostream"), m_severity(severity)
    {
        init(name);
    }
